...
function req_saveNP1(id_topic,nv_post)
{
        ...
        let query = ` SELECT * FROM ${config.mysqlTable_post}  WHERE  message = ?  and id_topic = ?  `; // ? une valeur qqconque
...
function req_saveNP2(titre,nv_post,id_topic,userId,formatted) //$_POST['nv_post'] ,$_POST['id_topic']
{
        //echo "ce post est nv dans le topic\n" ;
        //on l'insère
        let query = ` INSERT INTO  ${config.mysqlTable_post} (id,titre, message, date_message, id_topic,id_utilisateur) VALUES (?,?,?,?,?,?) ` ; // ? une valeur qqconque

    ...
    if(nv_sujet != "") {
        ...
        if (resultat.length == 0) {
            ...
            if (resultat1.length == 0)
            {
                ...
                const resultat2 = await request_saveNewTopic.req_saveNT2(userId,nv_sujet ,id_cours,formatted) ;
                console.log("num insertion",resultat2) ;
                val = await request_saveNewTopic.get_val_champ(config.mysqlTable_cours, 'nb_topics' ,id_cours) ; //await tres imporatnt sinon on console.log(val) affiche Promise { <pending> }
                console.log(val,val[0].nb_topics) ;
                updated = await request_saveNewTopic.update_val_champ(config.mysqlTable_cours, 'nb_topics' ,'date_dernier_message',val[0].nb_topics+1 ,formatted,id_cours) ;//await tres imporatnt sinon on console.log(val) affiche Promise { <pending> }
                console.log(updated) ;

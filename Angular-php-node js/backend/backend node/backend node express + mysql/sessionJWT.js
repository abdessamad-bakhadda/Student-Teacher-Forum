...
function createSessionJWT (userId) {
    ...
    // session.
    const jwtToken = sessionJWT.sign( //on signe
        //payload
        {
            userId: userId,
...
function createSessionCookie(req, res, payload) {
    ...
    if ((typeof payload.userId !== 'undefined') && 
        ...
        jwtToken = req.cookies.SESSIONID; //cookie de session JWT = token
    }
    else {
        // on crée un nouveau cookie
        jwtToken = createSessionJWT(payload.userId); //cookie de session JWT = token
    }
...
function decodeSessionCookie(req) {
    ...
    try {
        const token = sessionJWT.verify( // on verifie la signature
            sessionid,
            RSA_PUBLIC_KEY,
            {algorithms: ['RS256']});

    ...
    if(message != "") {
        ...
        if (resultat.length == 0) {
            ...
            if (resultat1.length == 0)
            {
                ...
                let last_key_topic = await request_saveNewTopic.get_last_key_from('posts')
                console.log(Number (last_key_topic[0])+1) ;
                await request_saveNewPost.req_saveNP2(Number (last_key_topic[0])+1,nv_post,message,id_topic,userId,formatted) ;
                ...
                let val = await get_val_champ(config.arangoDbCollection_topic, 'nb_posts' ,id_topic) ; //await tres imporatnt sinon on console.log(val) affiche Promise { <pending> }
                console.log(val,val[0]) ;
                let updated = await update_val_champ(config.arangoDbCollection_topic, 'nb_posts' ,'date_dernier_message',val[0]+1,formatted ,id_topic) ;//await tres imporatnt sinon on console.log(val) affiche Promise { <pending> }
                console.log(updated) ;

                //il reste à changer nb_posts dans la table cours quand on ajoute qq chose à la BD
                const Cours = await request_posts.recupere_cours_from_topic(id_topic) ;
                const id_cours = Cours[0].id_cours ;
                val = await get_val_champ(config.arangoDbCollection_cours, 'nb_posts' ,id_cours) ; //await tres imporatnt sinon on console.log(val) affiche Promise { <pending> }
                console.log(val,val[0]) ;
                updated = await update_val_champ(config.arangoDbCollection_cours, 'nb_posts' ,'date_dernier_message',val[0]+1 ,formatted,id_cours) ;//await tres imporatnt sinon on console.log(val) affiche Promise { <pending> }
                console.log(updated) ;
